...
if (count($_POST) > 0) {
	$opass=md5($_POST["currentPassword"]);
	$npass=md5($_POST['newPassword']);

    $result = mysqli_query($db, "SELECT * from users WHERE id='" . $_SESSION['user']['id'] . "'");
    $row = mysqli_fetch_array($result);
    if ($opass == $row["password"]) {
        mysqli_query($db, "UPDATE users set password='" . $npass . "' WHERE id='" . $_SESSION['user']['id'] . "'");
